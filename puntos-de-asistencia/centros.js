document.addEventListener('DOMContentLoaded', () => {
    ...
    // Dibuja los centros en el mapa
    function dibujarCentrosEnMapa() {
        // Comprobamos si el mapa existe ANTES de dibujar
        if (window.mapaExiste) {
            puntos.forEach(punto => {
                const [lat, lng] = punto.coordenadas.split(',').map(Number);
                // Ahora podemos llamar a crearCentro
                window.crearCentro(lat, lng, punto.nombre);
            });
        } else {
